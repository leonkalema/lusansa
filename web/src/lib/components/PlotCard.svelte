...
>
	{#if plot.photos?.[0]}
		<img
			...
			height="440"
			loading="lazy"
			class="aspect-[18/11] w-full object-cover"
		/>
	{:else}
		...
		<div class="flex aspect-[18/11] w-full flex-col justify-between bg-paper-dark p-4">
			<svg viewBox="0 0 80 44" class="h-10 w-auto self-end" fill="none" aria-hidden="true">
				<path d="M6,34 l30,-18 14,20 -30,18 z" stroke="#107800" stroke-opacity="0.7" stroke-width="1.5" fill="#107800" fill-opacity="0.08" transform="translate(8,-6)" />
				<rect x="12" y="26" width="3" height="3" fill="#107800" />
				<rect x="42" y="8" width="3" height="3" fill="#107800" />
				<rect x="56" y="28" width="3" height="3" fill="#107800" />
				<rect x="26" y="46" width="3" height="3" fill="#107800" transform="translate(0,-8)" />
			</svg>
			{#if plot.coordinates}
	...
	<div class="p-5">
		...
		<h3 class="text-xl text-ink group-hover:text-green-800">{plot.title}</h3>
		<p class="mt-1 text-[15px] text-ink-soft">
			{plot.sizeText} · {plot.district}{plot.nearestLandmark ? ` · ${plot.nearestLandmark}` : ''}
		</p>
		<p class="tabular mt-3 text-lg font-semibold text-ink">{ugxCompact(plot.priceUGX)}</p>
	</div>
</a>
